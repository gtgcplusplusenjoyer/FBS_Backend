    ...
    public class UpdateWorkoutDtoValidator : AbstractValidator<UpdateWorkoutDto>
    {
        ...
        public UpdateWorkoutDtoValidator()
        {
            ...
            RuleFor(x => x.Date)
                ...
                .WithMessage($"Cannot update workout to date earlier than {MaxPastMonths} month ago");
             
            RuleFor(x => x.Date)
                .LessThanOrEqualTo(DateOnly.FromDateTime(DateTime.UtcNow.AddMonths(MaxFutureMonths)))
                .WithMessage($"Cannot update workout to date more than {MaxFutureMonths} month in the future");
